...
public class JZMediaIjk extends JZMediaInterface implements IMediaPlayer.OnPreparedListener, IMediaPlayer.OnVideoSizeChangedListener, IMediaPlayer.OnCompletionListener, IMediaPlayer.OnErrorListener, IMediaPlayer.OnInfoListener, IMediaPlayer.OnBufferingUpdateListener, IMediaPlayer.OnSeekCompleteListener, IMediaPlayer.OnTimedTextListener, IjkMediaPlayer.OnNativeInvokeListener {
    ...
    public void prepare() {
        ...
        mMediaHandler.post(() -> {
            ...
            ijkMediaPlayer.setOption(IjkMediaPlayer.OPT_CATEGORY_FORMAT, "buffer_size", 1024 * 5);
            // 设置播放前的探测时间 1,达到首屏秒开效果
            ijkMediaPlayer.setOption(IjkMediaPlayer.OPT_CATEGORY_FORMAT, "analyzeduration", 1);
            // 最大缓存时长
            ijkMediaPlayer.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "max_cached_duration", 5); //300
            ...
            ijkMediaPlayer.setOnNativeInvokeListener(JZMediaIjk.this);

            try {
                if (jzvd.jzDataSource.getCurrentUrl().toString().startsWith("http://"))
                    ijkMediaPlayer.setDataSource("ijkhttphook:" + jzvd.jzDataSource.getCurrentUrl().toString());
                else
